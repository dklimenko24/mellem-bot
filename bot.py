...
# Функция расчёта розничной цены
def calculate_retail_price(wholesale_price: int) -> int:
    """Рассчитывает розничную цену с наценкой 30% и округлением вниз до ближайших 50."""
    retail = wholesale_price * 1.3
    retail = math.floor(retail / 50) * 50
    return int(retail)
...
def create_size_keyboard(prices_dict: dict) -> InlineKeyboardMarkup:
    ...
    for size, wholesale_price in prices_dict.items():
        ...
        # callback_data включает и тип материала, чтобы потом можно было различить
...
async def material_chosen(callback: types.CallbackQuery, state: FSMContext):
    ...
    if material == "keramika":
        ...
        # Здесь можно реализовать свою логику для "Своей формы"
...
# ----- Хендлер на выбор размера через кнопки -----
@dp.callback_query(lambda c: c.data.startswith("size_"), state=OrderState.waiting_for_size)
